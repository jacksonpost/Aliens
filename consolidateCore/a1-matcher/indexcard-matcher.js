...
var indexCardPath = 'data/d2375.json';
var matches = [];
...
fs.readFile(namesPath, 'utf-8', function (err, fileContents) {
	...
	fs.readFile(indexCardPath, 'utf-8', function (err, fileContents) {
	  	if (err) throw err;
	  	indexcard = JSON.parse(fileContents);
	  	console.log("loaded " + indexcard.length + " records");
		...
		console.log("filtered to " + indexcard.length + " naturalisation records");
...
*/
		...
		for (var i = 0; i < indexcard.length; i++) {
			var n = indexcard[i];
			...
			if (matches[0][0] > match_threshhold){
				...
				var match = {
					...
					'd2375title':n.controlSymbol,
				
					'd2375barcode':n.Barcode,
				...
				//matchedNames.push(n);
				console.log(matches[0][1] + ' - ' + n.controlSymbol + ' - ' + Number(matches[0][0]).toFixed(2));
			} 
		...
		var outputFilename = 'indexMatchnames.json';
